...
fn write_price_cell(buf: &mut String, p: Option<f64>, is_min: bool, is_max: bool) {
    match p {
        ...
        Some(v) => {
            let color = if is_min { "32" } else if is_max { "31" } else { "37" };
            if v >= 1000.0 {
                let _ = write!(buf, "\x1b[90m│\x1b[{}m{:<w$.2}\x1b[0m", color, v, w = COL_PRICE);
            } else if v >= 1.0 {
                let _ = write!(buf, "\x1b[90m│\x1b[{}m{:<w$.4}\x1b[0m", color, v, w = COL_PRICE);
            } else if v >= 0.01 {
                let _ = write!(buf, "\x1b[90m│\x1b[{}m{:<w$.6}\x1b[0m", color, v, w = COL_PRICE);
            } else if v >= 0.0001 {
                let _ = write!(buf, "\x1b[90m│\x1b[{}m{:<w$.8}\x1b[0m", color, v, w = COL_PRICE);
            } else {
                let _ = write!(buf, "\x1b[90m│\x1b[{}m{:<w$.10}\x1b[0m", color, v, w = COL_PRICE);
            }
        }
    }
}
...
fn write_spread_cell(buf: &mut String, spread: Option<f64>) {
    match spread {
        ...
        Some(s) => {
            ...
            let _ = write!(buf, "\x1b[90m│\x1b[{}m{:<w$}\x1b[0m", color, s_text, w = COL_SPREAD);
        }
    }
}
...
fn format_price(v: f64) -> String {
    if v >= 1000.0       { format!("{:.2}", v) }
    else if v >= 1.0     { format!("{:.4}", v) }
    else if v >= 0.01    { format!("{:.6}", v) }
    else if v >= 0.0001  { format!("{:.8}", v) }
    else                 { format!("{:.10}", v) }
}
...
fn status_indicator(connected: bool, enabled: bool, updates: u64) -> String {
    if !enabled { "⚪OFF".to_string() }
    else if connected { format!("🟢{}", updates) }
    else { "🔴".to_string() }
}
...
pub async fn run(
    ...
    loop {
            ...
            if kind == crossterm::event::KeyEventKind::Press || kind == crossterm::event::KeyEventKind::Repeat {
                    ...
                    if search_mode {
                        ...
                        match code {
                            ...
                            KeyCode::Char('1') => { let curr = status.binance_enabled.load(Ordering::Relaxed); status.binance_enabled.store(!curr, Ordering::Relaxed); }
                            KeyCode::Char('2') => { let curr = status.bybit_enabled.load(Ordering::Relaxed); status.bybit_enabled.store(!curr, Ordering::Relaxed); }
        ...
        for entry in store.iter() {
            ...
            if !bin_e { bin_val = None; }
            if !byb_e { byb_val = None; }
            ...
            let has_open = engine.try_lock()
                .map(|eng| eng.open_positions.contains_key(key.as_str()))
                .unwrap_or(false);

            let funding_hours = funding_store.get(key.as_str()).map(|r| *r.value()).unwrap_or(8);
        ...
        let position_details: Vec<PositionDetail> = if let Ok(eng) = engine.try_lock() {
            eng.open_positions.iter().map(|(coin, pos)| {
                let hold_secs = chrono::Utc::now().signed_duration_since(pos.open_time).num_seconds();
                ...
                let (cur_buy, cur_sell) = if let Some(prices) = store.get(coin) {
                    ...
                    (bp.unwrap_or(pos.entry_buy_price), sp.unwrap_or(pos.entry_sell_price))
                } else {
                ...
                let unrealized_pnl = long_pnl + short_pnl - pos.entry_buy_commission - pos.entry_sell_commission;
                PositionDetail {
                    coin: coin.clone(),
        ...
        let pos_panel_lines = if position_details.is_empty() { 0 } else { position_details.len() + 2 };
        ...
        // ── Status bar ──
        let bin_s = status_indicator(status.binance_connected.load(Ordering::Relaxed), bin_e, status.binance_updates.load(Ordering::Relaxed));
        let byb_s = status_indicator(status.bybit_connected.load(Ordering::Relaxed), byb_e, status.bybit_updates.load(Ordering::Relaxed));
        ...
        let status_line = format!(
            ...
            total_rows, chrono::Local::now().format("%H:%M:%S"),
            bin_s, byb_s, trade_indicator, search_display,
        );
        let _ = write!(buf, "\x1b[{row_idx};1H{}{}\x1b[K\x1b[0m", status_bg, status_line);
        row_idx += 1;
        ...
        // ── Trading info bar (always shown: shows live balances and trade limit) ──
        {
            ...
            if last_balance_refresh.elapsed() >= Duration::from_millis(500) {
                if let Ok(eng) = engine.try_lock() {
                    ...
                    cached_bin_bal = if bin_ws > 0.0 { bin_ws } else { eng.binance_balance };
                    cached_byb_bal = if byb_ws > 0.0 { byb_ws } else { eng.bybit_balance };
            ...
            match engine_guard {
                Ok(eng) => {
                    let bin_bal = if cached_bin_bal > 0.0 { cached_bin_bal } else { eng.binance_balance };
                    let byb_bal = if cached_byb_bal > 0.0 { cached_byb_bal } else { eng.bybit_balance };
                    let pnl_color = if eng.total_pnl > 0.0 { "32" } else if eng.total_pnl < 0.0 { "31" } else { "37" };
                    let open_count = eng.open_position_count();
                    let closed_count = eng.closed_count;
                    let unrealized = eng.total_unrealized_pnl(&store);
                    let unr_color = if unrealized > 0.0 { "32" } else if unrealized < 0.0 { "31" } else { "37" };
                    let halt_badge = if eng.emergency_halt { " \x1b[31;1m[🚨HALT-R to reset]\x1b[0m\x1b[100m" } else { "" };

                    let limit_badge = if trade_limit == 0 {
                        format!("Limit:∞ (taken:{})", trades_taken)
                    } else if limit_reached {
                        format!("\x1b[33;1mLimit:{}/{} [DONE]\x1b[0m\x1b[100m", trades_taken, trade_limit)
                    } else {
                        format!("Limit:{}/{}", trades_taken, trade_limit)
                    };

                    let last_trade = eng.recent_trades.back().map(|t| {
                        let type_icon = match t.trade_type {
                            crate::trade_journal::TradeType::Open => "📈",
                            crate::trade_journal::TradeType::Close => "📉",
                        };
                        format!("│ {} {} {}->{} \x1b[{}m${:.4}\x1b[0m\x1b[100m",
                            type_icon, t.coin, t.exchange_buy, t.exchange_sell,
                            if t.pnl_net > 0.0 { "32" } else if t.pnl_net < 0.0 { "31" } else { "37" },
                            t.pnl_net)
                    }).unwrap_or_default();
                    ...
                    let _ = write!(buf, "\x1b[{row_idx};1H\x1b[37;100m{}\x1b[K\x1b[0m", trade_bar);
                }
                Err(_) => {
                    let _ = write!(buf, "\x1b[{row_idx};1H\x1b[37;100m 💰 Loading balances...\x1b[K\x1b[0m");
                }
            }
            row_idx += 1;
        }
        ...
        if !position_details.is_empty() {
            ...
            for pd in &position_details {
                let pnl_color = if pd.unrealized_pnl > 0.0 { "32;1" } else if pd.unrealized_pnl < 0.0 { "31;1" } else { "37" };
            ...
            let _ = write!(
                buf, "\x1b[{row_idx};1H\x1b[90m{}\x1b[K\x1b[0m",
        ...
        // ── Header ──
        let _ = write!(
            buf, "\x1b[{row_idx};1H\x1b[37;100;1m{:<cw$}│{:<pw$}│{:<pw$}│{:<sw$}\x1b[K\x1b[0m",
            " COIN", "BINANCE ⚡", "BYBIT ⚡", "SPREAD %",
            cw = COL_COIN, pw = COL_PRICE, sw = COL_SPREAD,
        );
        row_idx += 1;
        ...
        let _ = write!(
            buf, "\x1b[{row_idx};1H\x1b[90m{}┼{}┼{}┼{}\x1b[K\x1b[0m",
            "─".repeat(COL_COIN), "─".repeat(COL_PRICE),
            "─".repeat(COL_PRICE), "─".repeat(COL_SPREAD),
        );
        row_idx += 1;
        ...
        for i in scroll_offset..end {
            ...
            let _ = write!(buf, "\x1b[{row_idx};1H\x1b[{}m{:<w$}\x1b[0m", cc, coin_display, w = COL_COIN);

            write_price_cell(&mut buf, row.binance, min_idx == Some(0), max_idx == Some(0));
