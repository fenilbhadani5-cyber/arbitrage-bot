...
use price_store::{new_funding_store, new_status, new_store, Exchange};
...
fn print_trade_summary() {
    ...
    if trades.is_empty() {
        ...
        println!(
            "  Trade log:        \x1b[90m{}\x1b[0m",
            LIVE_TRADES_LOG_PATH
        );
        println!(
            "  Missed trade log: \x1b[90m{}\x1b[0m\n",
    ...
    let pnl_color = if summary.total_pnl_net > 0.0 {
        "32;1"
    } else {
        "31;1"
    };
    ...
    println!(
        "  \x1b[32mWins:\x1b[0m          {}  │  \x1b[31mLosses:\x1b[0m  {}",
        summary.winning_trades, summary.losing_trades
    );
    println!("  \x1b[37;1mWin Rate:\x1b[0m      {:.1}%", summary.win_rate);
    println!();
    println!(
        "  \x1b[37;1mGross PnL:\x1b[0m     \x1b[{}m${:.4}\x1b[0m",
        pnl_color, summary.total_pnl_gross
    );
    println!(
        "  \x1b[37;1mTotal Fees:\x1b[0m     \x1b[33m-${:.4}\x1b[0m",
        summary.total_fees
    );
    println!(
        "  \x1b[37;1mNet PnL:\x1b[0m       \x1b[{}m${:.4}\x1b[0m",
        pnl_color, summary.total_pnl_net
    );
    println!(
        "  \x1b[37;1mAvg Spread:\x1b[0m    {:.4}%",
        summary.avg_spread_captured
    );
    println!(
        "  \x1b[37;1mBest Trade:\x1b[0m    \x1b[32m${:.4}\x1b[0m",
        summary.best_trade_pnl
    );
    println!(
        "  \x1b[37;1mWorst Trade:\x1b[0m   \x1b[31m${:.4}\x1b[0m",
    ...
    println!(
        "  \x1b[37;1m{:<12} {:>8} {:>8} {:>12} {:>12}\x1b[0m",
        "Exchange", "Buy", "Sell", "Fees Paid", "PnL"
    );
    println!("  \x1b[90m{}\x1b[0m", "─".repeat(58));

    for ex in &[Exchange::Binance, Exchange::Bybit] {
        if let Some(stats) = summary.exchange_stats.get(ex) {
            let pnl_c = if stats.total_pnl_contribution > 0.0 {
                "32"
            } else {
                "31"
            };
            println!(
                "  {:<12} {:>8} {:>8} \x1b[33m${:>10.4}\x1b[0m \x1b[{}m${:>10.4}\x1b[0m",
                ex,
                stats.trades_as_buy,
                stats.trades_as_sell,
                stats.total_fees_paid,
                pnl_c,
                stats.total_pnl_contribution
            );
        }
    }

    // Last 10 trades
    let recent_start = if trades.len() > 10 {
        trades.len() - 10
    } else {
        0
    };
    ...
    println!(
        "  \x1b[37;1m{:<6} {:<8} {:<10} {:<10} {:>10} {:>10} {:>8} {:>10}\x1b[0m",
    ...
    println!(
        "\n  \x1b[90mFull trade log:   {}\x1b[0m",
        LIVE_TRADES_LOG_PATH
    );
    println!(
        "  \x1b[90mMissed trade log: {}\x1b[0m",
        MISSED_TRADES_LOG_PATH
    );
    println!();
}
...
fn load_env_file() {
    ...
    for path in &candidates {
        if let Ok(content) = std::fs::read_to_string(path) {
            ...
            if loaded_count > 0 {
                println!(
                    "\x1b[32m[Config] Loaded {} API keys from {}\x1b[0m",
                    loaded_count, path
                );
            }
            break;
...
async fn main() {
    ...
    let binance_api_secret = std::env::var("BINANCE_API_SECRET").unwrap_or_else(|_| {
        eprintln!(
            "\x1b[31;1m[ERROR] BINANCE_API_SECRET not set in environment or keys.env!\x1b[0m"
        );
        String::new()
    });
    ...
    if binance_api_key.is_empty()
        || binance_api_secret.is_empty()
        || bybit_api_key.is_empty()
        || bybit_api_secret.is_empty()
    {
        eprintln!("\n\x1b[31;1m[FATAL] All 4 API keys must be set. Exiting.\x1b[0m");
        eprintln!(
            "  Please open \x1b[33;1mkeys.env\x1b[0m in the project folder and paste your keys:"
        );
    ...
    use exchanges::fill_channel::{new_live_balance, new_pending_fill_map};
    ...
    match binance_client.get_balance().await {
        ...
        Err(e) => {
            eprintln!(
                "\x1b[31;1m[Startup] Binance API key verification FAILED: {}\x1b[0m",
                e
            );
    ...
    match bybit_client.get_balance().await {
        ...
        Err(e) => {
            eprintln!(
                "\x1b[31;1m[Startup] Bybit API key verification FAILED: {}\x1b[0m",
                e
            );
    ...
    match bin_info_result {
        Ok(count) => eprintln!(
            "[Startup] Binance: loaded {} symbol specs (tick/step/notional)",
            count
        ),
        Err(e) => eprintln!(
            "[Startup] WARNING: Failed to load Binance exchangeInfo: {} — using heuristic rounding",
            e
        ),
    }
    ...
    // ── Load historical trades & sync real account positions ──
    {
        ...
        if !historical.is_empty() {
            ...
            eprintln!(
                "[Startup] Loaded {} historical trades from {}",
                historical.len(),
                LIVE_TRADES_LOG_PATH
            );
            ...
            let start = if historical.len() > 100 {
                historical.len() - 100
            } else {
                0
            };
    ...
    tokio::spawn(async move {
        loop {
            exchanges::bybit_feed::run(byb_store.clone(), byb_status.clone(), byb_funding.clone())
                .await;
    ...
    tokio::spawn(async move {
        live_trading::run_trading_loop(trade_store, trade_status, trade_engine, trade_funding)
            .await;
    });
